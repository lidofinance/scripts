...
def add_evmscript_factory(factory: Contract | str, permissions: str) -> Tuple[str, str]:
    ...
    return (easy_track.address, easy_track.addEVMScriptFactory.encode_input(factory, permissions))


def remove_evmscript_factory(factory: Contract | str) -> Tuple[str, str]:
    easy_track = contracts.easy_track

    return (easy_track.address, easy_track.removeEVMScriptFactory.encode_input(factory))


def create_permissions_for_overloaded_method(
    contract: Contract, method: str, paramethers: Tuple[str, ...] = None
) -> str:
...
def create_permissions_overloaded(contract: Contract, method: str, func_args: str) -> str:
    return contract.address + getattr(contract, method)[func_args].signature[2:]


def set_motions_count_limit(motionsCountLimit: int) -> Tuple[str, str]:
    easy_track = contracts.easy_track

    return (easy_track.address, easy_track.setMotionsCountLimit.encode_input(motionsCountLimit))
