...
def assert_new_state(step, diff):
    ...
    assert_expected_diffs(step, diff, {
        'getELRewardsWithdrawalLimit()': ValueChanged(from_val=0, to_val=2),
        'getBeaconReportReceiver()': ValueChanged(from_val=ZERO_ADDRESS, to_val=composite_receiver.address),
        'callbacksLength()': ValueChanged(from_val=0, to_val=1),
        'callbacks(0)': ValueChanged(from_val=None, to_val=self_owned_steth_burner.address)
    })
