...
from archive.scripts.vote_2023_12_18 import start_vote, TokenLimit, amount_limits
...
def test_vote(helpers, accounts, vote_ids_from_env, stranger, bypass_events_decoding):
    ...
    rcc_payout = Payout(
        token_addr=steth.address, from_addr=agent.address, to_addr=rcc_multisig_acc.address, amount=224 * 10**18
    )

    pml_steth_balance_before = steth.balanceOf(pml_multisig_acc)
    pml_payout = Payout(
        token_addr=steth.address, from_addr=agent.address, to_addr=pml_multisig_acc.address, amount=359 * 10**18
    )

    atc_steth_balance_before = steth.balanceOf(atc_multisig_acc)
    atc_payout = Payout(
        token_addr=steth.address, from_addr=agent.address, to_addr=atc_multisig_acc.address, amount=314 * 10**18
    )
    ...
    # 9. Transfer 224 stETH to RCC 0xDE06d17Db9295Fa8c4082D4f73Ff81592A3aC437
    ...
    # 10. Transfer 359 stETH to PML 0x17F6b2C738a63a8D3A113a228cfd0b373244633D
    ...
    # 11. Transfer 314 stETH to ATC 0x9B1cebF7616f2BC73b47D226f90b01a7c9F86956
