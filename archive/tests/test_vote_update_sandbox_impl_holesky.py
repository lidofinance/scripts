...
from scripts.vote_update_sandbox_impl_holesky import start_vote
...
def validate_app_set_event(event: EventDict, app_id: str, app_addr: str, emitted_by: Optional[str] = None):
    ...
    assert event["SetApp"][0]["appId"] == app_id_bytes, "Wrong app ID"
    assert event["SetApp"][0]["appAddress"] == app_addr, "Wrong app address"
