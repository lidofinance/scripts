...
def test_dsm_upgrade(helpers, accounts, ldo_holder, dao_voting, vote_id_from_env, dao_agent, lido) -> None:
    ...
    assert (
        new_dsm.getLastDepositBlock() == 0
    ), "new DSM: last deposit block should be zero initially"  # should be uninitialized
    ...
    assert (
        old_dsm.getLastDepositBlock() == old_dsm_last_deposited_block
    ), "old DSM: vote should preserve last deposited block"
    assert (
        new_dsm.getLastDepositBlock() == new_dsm_last_deposited_block
    ), "new DSM: vote should update last deposit block properly"
...
def assert_settings_are_same(
    old_dsm: interface.DepositSecurityModule, new_dsm: interface.DepositSecurityModule
) -> None:
