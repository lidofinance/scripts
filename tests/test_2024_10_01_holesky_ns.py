...
def test_vote(helpers, accounts, vote_ids_from_env, stranger, ldo_holder, bypass_events_decoding):
    ...
    alliance_ops_top_up_evm_script_factory_new = "0x79e0286a8f60276293a7D29f53979642CcBD4Fff" # TopUpAllowedRecipients
    assert alliance_ops_top_up_evm_script_factory_new not in evm_script_factories_before

    stonks_multisig_acc = accounts.at("xd4090CA1134F8dE1450B8246916F73d212efdEf6", force=True) # Testnet DAO Multisigs
    stonks_dai_usdc_contract = accounts.at("0x2eb8e9198e647f80ccf62a5e291bcd4a5a3ca68c", force=True) # Testnet Stablecoins
    stonks_usdc_dai_contract = accounts.at("0x9715b2786f1053294fc8952df923b95cab9aac42", force=True)
    stonks_usdt_dai_contract = accounts.at("0x86f6c353a0965eb069cd7f4f91c1afef8c725551", force=True)
    ...
    alliance_ops_allowed_recipients_registry = interface.AllowedRecipientRegistry(
        "0x94499410c47FF7572560Ef8CBAf68bCdd0F38D08"
    )
    check_add_and_remove_recipient_with_voting(
