"""
Tests for voting 13/12/2022.
"""

from brownie.network.transaction import TransactionReceipt
from scripts.vote_2022_12_13 import start_vote
...
total_ldo_vesting_amount: int = ldo_vesting_amount * 2
...
token_manager_ldo_payout = Payout(
    ...
    amount=total_ldo_vesting_amount,
)
...
def test_vote(
    ...
    assert agent_ldo_after == agent_ldo_before - total_ldo_vesting_amount
    ...
    no = node_operators_registry.getNodeOperator(new_node_op.id, True)

    message = f"Failed on {new_node_op.name}"
    assert no[0] is True, message  # is active
    assert no[1] == new_node_op.name, message  # name
    assert no[2] == new_node_op.reward_address, message  # rewards address
    ...
    validate_node_operator_added_event(evs[3], new_node_op)
