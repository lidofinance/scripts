...
def test_coverage_application_on_zero_rewards_report(helpers, vote_ids_from_env, accounts, steth_whale):
    ...
    cover_shares_burnt_before = contracts.burner.getCoverSharesBurnt()
    ...
    assert (
        ...
        == shares_burnt["sharesAmount"] - withdrawals_finalized["sharesToBurn"] + cover_shares_burnt_before
    )
    assert (
        contracts.burner.getCoverSharesBurnt(block_identifier=block_after_report)
        == contracts.burner.getSharesRequestedToBurn(block_identifier=block_before_report)[COVER_INDEX]
        + cover_shares_burnt_before
    )
    assert (
...
def test_coverage_application_on_nonzero_rewards_report(helpers, vote_ids_from_env, accounts, steth_whale):
    ...
    with chain_snapshot():
        ...
        assert contracts.burner.getCoverSharesBurnt() == cover_shares_burnt_before
    ...
    assert (
        shares_burnt_overall == no_coverage_shares_burnt_overall + coverage_shares_to_burn + cover_shares_burnt_before
    )
    ...
    assert treasury_shares_after_report == no_coverage_treasury_shares_after_report - coverage_shares_to_burn
