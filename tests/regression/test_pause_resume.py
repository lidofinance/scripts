...
from utils.test.helpers import almostEqEth, almostEqWithDiff, topped_up_contract
...
class TestEventsEmitted:
    def test_stop_resume_lido_emit_events(self, helpers):
        tx = contracts.lido.stop({"from": topped_up_contract(contracts.voting)})
        ...
        tx = contracts.lido.resume({"from": topped_up_contract(contracts.voting)})
    ...
    def test_stop_resume_staking_lido_emit_events(self, helpers):
        tx = contracts.lido.pauseStaking({"from": topped_up_contract(contracts.voting)})
        helpers.assert_single_event_named("StakingPaused", tx, {})
        ...
        tx = contracts.lido.resumeStaking({"from": topped_up_contract(contracts.voting)})
    ...
    def test_pause_resume_deposits_staking_module(self, helpers, stranger):
        tx = contracts.staking_router.pauseStakingModule(1, {"from": topped_up_contract(contracts.deposit_security_module)})
        ...
        tx = contracts.staking_router.resumeStakingModule(1, {"from": topped_up_contract(stranger)})
        helpers.assert_single_event_named(
            "StakingModuleStatusSet",
            tx,
    ...
    def test_stop_staking_module(self, helpers, stranger):
        ...
        tx = contracts.staking_router.setStakingModuleStatus(1, StakingModuleStatus.Stopped, {"from": topped_up_contract(stranger)})
        helpers.assert_single_event_named(
            "StakingModuleStatusSet",
            tx,
    ...
    def test_pause_resume_withdrawal_queue(self, helpers, stranger):
        ...
        tx = contracts.withdrawal_queue.pauseFor(inf, {"from": topped_up_contract(stranger)})
        ...
        tx = contracts.withdrawal_queue.resume({"from": topped_up_contract(stranger)})
        helpers.assert_single_event_named("Resumed", tx, {})
    ...
    def test_pause_resume_validators_exit_bus(self, helpers, stranger):
        ...
        tx = contracts.validators_exit_bus_oracle.pauseFor(inf, {"from": topped_up_contract(stranger)})
        helpers.assert_single_event_named("Paused", tx, {"duration": inf})
        ...
        tx = contracts.validators_exit_bus_oracle.resume({"from": topped_up_contract(stranger)})
        helpers.assert_single_event_named("Resumed", tx, {})
...
class TestRevertedSecondCalls:
    ...
    def test_revert_second_pause_resume_staking(self):
        contracts.lido.pauseStaking({"from": topped_up_contract(contracts.voting)})

        with brownie.reverts(""):
            contracts.lido.pauseStaking({"from": topped_up_contract(contracts.voting)})

        contracts.lido.resumeStaking({"from": topped_up_contract(contracts.voting)})
    ...
    def test_revert_second_stop_staking_module(self, helpers, stranger):
        ...
        contracts.staking_router.setStakingModuleStatus(1, StakingModuleStatus.Stopped, {"from": topped_up_contract(stranger)})
    ...
    def test_revert_second_pause_resume_validators_exit_bus(self, helpers, stranger):
        ...
        contracts.validators_exit_bus_oracle.pauseFor(inf, {"from": topped_up_contract(stranger)})
        with brownie.reverts(encode_error("ResumedExpected()")):
            contracts.validators_exit_bus_oracle.pauseFor(inf, {"from": topped_up_contract(stranger)})
        ...
        contracts.validators_exit_bus_oracle.resume({"from": topped_up_contract(stranger)})
        with brownie.reverts(encode_error("PausedExpected()")):
            contracts.validators_exit_bus_oracle.resume({"from": topped_up_contract(stranger)})
...
def test_stopped_lido_cant_deposit():
    with brownie.reverts("CAN_NOT_DEPOSIT"):
        contracts.lido.deposit(1, 1, "0x", {"from": topped_up_contract(contracts.deposit_security_module)}),
...
def make_withdrawal_request(stranger):
    stranger.transfer(contracts.lido, DEPOSIT_AMOUNT)
    contracts.lido.approve(contracts.withdrawal_queue, DEPOSIT_AMOUNT - 1, {"from": topped_up_contract(stranger)})
    contracts.withdrawal_queue.requestWithdrawals([DEPOSIT_AMOUNT - 1], stranger, {"from": topped_up_contract(stranger)})
