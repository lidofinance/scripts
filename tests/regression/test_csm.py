...
from utils.test.staking_router_helpers import (
    ...
    StakingModuleStatus,
    increase_staking_module_share, get_staking_module_status,
)

contracts: ContractsLazyLoader = contracts

CSM_MODULE_ID = 4  # TODO: it is for tests on holesky
MAX_DEPOSITS = 50
...
def pause_modules():
    ...
    for module in modules:
        if module[0] != CSM_MODULE_ID:
            if get_staking_module_status(module[0]) == StakingModuleStatus.Stopped:
                continue
...
def test_eject_bad_performer(csm, ejector, node_operator):
    ...
    assert "WithdrawalRequestAdded" in tx.events
