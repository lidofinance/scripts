...
from utils.test.helpers import ONE_ETH, topped_up_contract
...
def test_is_staking_not_paused(voting):
    contracts.lido.resumeStaking({"from": topped_up_contract(voting)})
...
def test_pause_staking_access(voting, stranger):
    ...
    with reverts("APP_AUTH_FAILED"):
        contracts.lido.pauseStaking({"from": topped_up_contract(stranger)})
    contracts.lido.pauseStaking({"from": topped_up_contract(voting)})


def test_pause_staking_works(voting, stranger):
    contracts.lido.resumeStaking({"from": topped_up_contract(voting)})
    # Should not allow to stake until it's paused
    tx = contracts.lido.pauseStaking({"from": topped_up_contract(voting)})

    assert len(tx.logs) == 1
    assert_staking_is_paused(tx.logs[0])
    with reverts("STAKING_PAUSED"):
        contracts.lido.submit(ZERO_ADDRESS, {"from": topped_up_contract(stranger), "amount": ONE_ETH})


def test_resume_staking_access(voting, stranger):
    # Should not allow to resume staking from unauthorized account
    with reverts("APP_AUTH_FAILED"):
        contracts.lido.resumeStaking({"from": topped_up_contract(stranger)})
    contracts.lido.resumeStaking({"from": topped_up_contract(voting)})
...
def test_resume_staking_works(voting, stranger):
    ...
    tx = contracts.lido.resumeStaking({"from": topped_up_contract(voting)})
...
def test_set_staking_limit_access(voting, stranger):
    # Should not allow to resume staking from unauthorized account
    with reverts("APP_AUTH_FAILED"):
        contracts.lido.setStakingLimit(ONE_ETH, ONE_ETH * 0.01, {"from": topped_up_contract(stranger)})
    contracts.lido.setStakingLimit(ONE_ETH, ONE_ETH * 0.01, {"from": topped_up_contract(voting)})
...
def test_staking_limit_updates_per_block_correctly(voting, stranger, limit_max, limit_per_block):
    ...
    contracts.lido.setStakingLimit(limit_max, limit_per_block, {"from": topped_up_contract(voting)})
    staking_limit_before = contracts.lido.getCurrentStakeLimit()
    assert limit_max == staking_limit_before
    contracts.lido.submit(ZERO_ADDRESS, {"from": topped_up_contract(stranger), "amount": limit_per_block})
...
def test_staking_limit_is_zero(voting):
    ...
    with reverts("ZERO_MAX_STAKE_LIMIT"):
        contracts.lido.setStakingLimit(0, 0, {"from": topped_up_contract(voting)})
...
def test_staking_limit_is_uint256(voting):
    ...
    with reverts("TOO_LARGE_MAX_STAKE_LIMIT"):
        contracts.lido.setStakingLimit(max_uint256, max_uint256, {"from": topped_up_contract(voting)})
...
def test_staking_limit_exceed(voting, stranger):
    ...
    with reverts("STAKE_LIMIT"):
        contracts.lido.submit(ZERO_ADDRESS, {"from": topped_up_contract(stranger), "amount": ONE_ETH * 10})


def test_remove_staking_limit_access(voting, stranger):
    # Should not allow to resume staking from unauthorized account
    with reverts("APP_AUTH_FAILED"):
        contracts.lido.removeStakingLimit({"from": topped_up_contract(stranger)})
    contracts.lido.removeStakingLimit({"from": topped_up_contract(voting)})
...
def test_protocol_pause(voting, stranger):
    ...
    contracts.lido.stop({"from": topped_up_contract(voting)})
...
def test_protocol_resume_after_pause(voting, stranger):
    ...
    contracts.lido.resume({"from": topped_up_contract(voting)})
    contracts.lido.submit(ZERO_ADDRESS, {"from": topped_up_contract(stranger), "amount": ONE_ETH})
...
def test_staking_limit_full_info(stranger):
    (
        ...
        prev_limit,
        block_number,
    ) = contracts.lido.getStakeLimitFullInfo({"from": topped_up_contract(stranger)})
...
class TestEventsEmitted:
    def test_staking_limit_emit_events(self, helpers):
        tx = contracts.lido.setStakingLimit(1000, 100, {"from": topped_up_contract(contracts.voting)})
    ...
    def test_staking_limit_change_emit_events(self, helpers):
        contracts.lido.setStakingLimit(1000, 100, {"from": topped_up_contract(contracts.voting)})
        tx = contracts.lido.setStakingLimit(2000, 200, {"from": topped_up_contract(contracts.voting)})
        helpers.assert_single_event_named(
    ...
    def test_staking_limit_remove_emit_events(self, helpers):
        ...
        tx = contracts.lido.removeStakingLimit({"from": topped_up_contract(contracts.voting)})
