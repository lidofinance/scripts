...
def test_setup_coverage(
    ...
    assert (
        oracle.getBeaconReportReceiver() == composite_post_rebase_beacon_receiver.address
    ), "Incorrect old beacon report receiver"
    assert oracle.getLido() == lido.address, "Incorrect lido address"
    assert lido.getOracle() == oracle.address, "Incorrect oracle address"
    assert composite_post_rebase_beacon_receiver.VOTING() == dao_voting.address, "Incorrect voting"

    assert composite_post_rebase_beacon_receiver.ORACLE() == oracle.address, "Incorrect oracle"
    assert composite_post_rebase_beacon_receiver.callbacksLength() == 1, "Incorrect callbacks length"
    ...
    assert composite_post_rebase_beacon_receiver.ORACLE() == oracle.address, "Incorrect oracle"
    assert composite_post_rebase_beacon_receiver.callbacksLength() == 1, "Incorrect callbacks length"
    assert composite_post_rebase_beacon_receiver.callbacks(0) == self_owned_steth_burner.address, "Incorrect callback"
    ...
    assert not has_burn_role_permission(
        acl, lido, self_owned_steth_burner, dao_agent.address, 100
    ), "Incorrect permissions"
    ...
    cover_application_acceptance_checks(self_owned_steth_burner, dao_voting, oracle, lido, dao_agent, network, accounts)

    burn_permissions_forehead_check(lido, self_owned_steth_burner, dao_agent, dao_voting)
...
def cover_application_acceptance_checks(steth_burner, dao_voting, oracle, lido, agent, network, accounts):
    ...
    assert lido.balanceOf(steth_burner.address) == 0, "Incorrect balance"
