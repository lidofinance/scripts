...
EXPECTED_SNAPSHOT_DIFFS: dict[str, Any] = {
    ...
    "lido.Lido.lidoLocator": ZERO_BYTES32,
    ...
    for v1_frame, v2_frame in zip(*stacks, strict=True):
        with check:
            ...
            for key, before_val in v1_frame["snap"].items():
                if key == "lido.Lido.bufferedEtherAndDepositedValidators":
                    if not _combined_buffered_and_deposited_slot_ok(
                        v1_frame["snap"],
                        v2_frame["snap"],
                    ):
                        unexpected[key] = (
                            v1_frame["snap"]["lido.Lido.bufferedEtherAndDepositedValidators"],
                            v2_frame["snap"]["lido.Lido.bufferedEtherAndDepositedValidators"],
                        )
                    continue

                if key == "lido.Lido.clBalanceAndClValidators":
                    if not _combined_cl_balance_and_validators_slot_ok(
                        v1_frame["snap"],
                        v2_frame["snap"],
                    ):
                        unexpected[key] = (
                            v1_frame["snap"]["lido.Lido.clBalanceAndClValidators"],
                            v2_frame["snap"]["lido.Lido.clBalanceAndClValidators"],
                        )
                    continue

                if key == "lido.StETH.totalAndExternalShares":
                    if not _combined_steth_total_and_external_shares_slot_ok(
                        v1_frame["snap"],
                        v2_frame["snap"],
                    ):
                        unexpected[key] = (
                            v1_frame["snap"]["lido.StETH.totalAndExternalShares"],
                            v2_frame["snap"]["lido.StETH.totalAndExternalShares"],
                        )
                    continue

                if key == "lido.Lido.lidoLocatorAndMaxExternalRatio":
                    if not _combined_lido_locator_and_max_external_ratio_slot_ok(
                        v1_frame["snap"],
                        v2_frame["snap"],
                    ):
                        unexpected[key] = (
                            v1_frame["snap"]["lido.Lido.lidoLocatorAndMaxExternalRatio"],
                            v2_frame["snap"]["lido.Lido.lidoLocatorAndMaxExternalRatio"],
                        )
                    continue
...
def _combined_buffered_and_deposited_slot_ok(
    v1_snap: dict[str, Any],
    v2_snap: dict[str, Any],
) -> bool:
    """
    Since v3, buffered ether and deposited validators are packed into one slot:
    |------ 128 bit -------|------ 128 bit -------|
    | deposited validators |    buffered ether    |
    """
    before_combined = v1_snap["lido.Lido.bufferedEtherAndDepositedValidators"]
    after_combined = v2_snap["lido.Lido.bufferedEtherAndDepositedValidators"]

    # Before upgrade the new slot must be empty
    if before_combined != ZERO_BYTES32:
        return False

    # After upgrade the new slot packs high 128 bits as deposited validators
    # and low 128 bits as buffered ether from the old slots.
    old_buffered = int.from_bytes(
        v1_snap["lido.Lido.bufferedEther"],
        byteorder="big",
    )
    old_deposited = int.from_bytes(
        v1_snap["lido.Lido.depositedValidators"],
        byteorder="big",
    )
    combined_after = int.from_bytes(after_combined, byteorder="big")
    low_mask = (1 << 128) - 1
    buffered_after = combined_after & low_mask
    deposited_after = combined_after >> 128

    return buffered_after == old_buffered and deposited_after == old_deposited


def _combined_cl_balance_and_validators_slot_ok(
    v1_snap: dict[str, Any],
    v2_snap: dict[str, Any],
) -> bool:
    """
    Since v3, CL balance and CL validators are packed into one slot:
    |----- 128 bit -----|------ 128 bit -------|
    |   CL validators   |     CL balance       |
    """
    before_combined = v1_snap["lido.Lido.clBalanceAndClValidators"]
    after_combined = v2_snap["lido.Lido.clBalanceAndClValidators"]

    # Before upgrade the new slot must be empty
    if before_combined != ZERO_BYTES32:
        return False

    # After upgrade the new slot packs high 128 bits as validators count
    # and low 128 bits as CL balance from the old slots.
    old_balance = int.from_bytes(
        v1_snap["lido.Lido.beaconBalance"],
        byteorder="big",
    )
    old_validators = int.from_bytes(
        v1_snap["lido.Lido.beaconValidators"],
        byteorder="big",
    )
    combined_after = int.from_bytes(after_combined, byteorder="big")
    low_mask = (1 << 128) - 1
    balance_after = combined_after & low_mask
    validators_after = combined_after >> 128

    return balance_after == old_balance and validators_after == old_validators


def _combined_steth_total_and_external_shares_slot_ok(
    v1_snap: dict[str, Any],
    v2_snap: dict[str, Any],
) -> bool:
    """
    Since v3, total shares and external shares are packed into one slot:
    |------ 128 bit -------|------ 128 bit -------|
    |   external shares    |     total shares     |
    """
    before_combined = v1_snap["lido.StETH.totalAndExternalShares"]
    after_combined = v2_snap["lido.StETH.totalAndExternalShares"]
    ...
    combined_after = int.from_bytes(after_combined, byteorder="big")
    low_mask = (1 << 128) - 1
    total_after_slot = combined_after & low_mask
    external_after = combined_after >> 128

    # External shares must be zero after upgrade
    if external_after != 0:
        return False

    # Total shares (low 128 bits) must equal the old storage slot value
    total_shares_before_slot = int.from_bytes(
        v1_snap["lido.StETH.totalShares"],
        byteorder="big",
    )
    return total_after_slot == total_shares_before_slot


def _combined_lido_locator_and_max_external_ratio_slot_ok(
    v1_snap: dict[str, Any],
    v2_snap: dict[str, Any],
) -> bool:
    """
    Since v3, max external ratio and lido locator are packed into one slot:
    |----- 96 bit -----|------ 160 bit -------|
    |max external ratio| lido locator address |
    """
    before_combined = v1_snap["lido.Lido.lidoLocatorAndMaxExternalRatio"]
    after_combined = v2_snap["lido.Lido.lidoLocatorAndMaxExternalRatio"]
    ...
    # Low 160 bits is locator address, high 96 bits is max external ratio BP
    locator_mask = (1 << 160) - 1
    locator_after = combined_after & locator_mask
    max_external_ratio_after = combined_after >> 160

    if max_external_ratio_after != INITIAL_MAX_EXTERNAL_RATIO_BP:
        return False

    # Locator address in the combined slot must match the old locator slot value
    before_locator_int = int.from_bytes(v1_snap["lido.Lido.lidoLocator"], byteorder="big")
    return locator_after == before_locator_int
